...
begin
  ...
  if v_tx.transaction_type = 'income' then
    ...
    if v_tx.envelope_id is not null then
      ...
      set remaining_amount = remaining_amount + v_tx.envelope_amount_used,
          overspent_amount = greatest(
            overspent_amount - v_wallet_debits, 0
          ),
          updated_at = now()
    ...
    where transaction_id = v_tx.id and user_id = p_user_id;
    delete from public.finance_notifications
    where transaction_id = v_tx.id and user_id = p_user_id;
  end if;
