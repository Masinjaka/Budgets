...
- [ ] Modify `transaction_detail_bottom_sheet.dart` to accept a `TransactionModel` object.
- [ ] Remove the `subcategoriesProvider`.
- [ ] Use the `subcategoryExpensesProvider` with the `transaction.id` to fetch and display the list of subcategory transactions.
