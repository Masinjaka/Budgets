...
class SupabaseProfileDataSource {
  ...
  Future<void> updateUserProfilePhoto({required String userId, required String photoUrl}) async {
    try {
      debugPrint('Updating profile photo URL for userId: $userId to $photoUrl');
      await client.from('user').update({'profile_photo': photoUrl}).eq('user_id', userId);
    } catch (e,s) {
