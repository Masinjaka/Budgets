...
class AiEntryService {
  ...
  Future<int> totalFunds() async {
    ...
    final values = await Future.wait([
      _client.from('wallets').select('balance').eq('user_id', userId),
      _client
          .from('envelopes')
          .select('remaining_amount')
          .eq('user_id', userId),
    ]);
    final wallets = values[0].fold<int>(
      0,
      (sum, row) => sum + (row['balance'] as num? ?? 0).round(),
    );
    final envelopes = values[1].fold<int>(
      0,
      (sum, row) => sum + (row['remaining_amount'] as num? ?? 0).round(),
    );
    return wallets + envelopes;
  }
